...
public final class SphereSettingsScreen extends Screen {
    ...
    private SphereBand selectedBand = SphereBand.R250_200;
    ...
    private void buildSphereWidgets() {
        ...
        addDrawableChild(ButtonWidget.builder(Text.literal("使用玩家坐标"), button -> usePlayerPosition())
                ...
                .build());
        addDrawableChild(ButtonWidget.builder(Text.literal("半径：" + selectedBand.label()), button -> {
    ...
    private void buildDisplayWidgets() {
        ...
        addDrawableChild(ButtonWidget.builder(
                        Text.literal("区域运算：" + snapshot.mode().chineseName()),
                        button -> toggleMode())
                .dimensions(contentX, y, Math.min(250, contentWidth), 20)
                .build());
        y += 31;
        ...
        try {
            ...
            switch (result) {
                case ADDED -> {
                    ...
                    if (updated.mode() == OperationMode.DELAYED_INTERSECTION && updated.spheres().size() < 3) {
                        setStatus("球体已记录并参与计算；达到 3 个球体后开始渲染（当前 "
                                + updated.spheres().size() + "/3）", false);
                    } else {
                        setStatus("球体已生成", false);
                    }
                    rebuildWidgets();
                }
    ...
    private void toggleMode() {
        ...
        OperationMode next = modes[(snapshot.mode().ordinal() + 1) % modes.length];
        ModConfig.ModeResult result = ModConfig.trySetMode(next);
        if (result == ModConfig.ModeResult.NO_COMMON_INTERSECTION) {
            setStatus("当前球体不存在共同交集，无法切换到该模式", true);
            return;
        }
        if (result == ModConfig.ModeResult.CHANGED) {
            SkeletonSearcherClient.RENDERER.markDirty();
            setStatus("已切换为" + next.chineseName() + "模式", false);
            rebuildWidgets();
        }
    }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float deltaTicks) {
        ...
        if (!status.isBlank()) {
            ...
            context.drawTextWithShadow(textRenderer, status, contentX, panelY + panelHeight - 23, color);
        }
    ...
    private void renderSpherePage(DrawContext context) {
        ...
        context.drawTextWithShadow(textRenderer,
                "已生成球体：" + spheres.size() + "   当前运算：" + snapshot.mode().chineseName(),
    ...
    private void renderDisplayPage(DrawContext context) {
        int y = panelY + 64;
        context.drawTextWithShadow(textRenderer, "高亮显示", contentX, y, COLOR_ORANGE);
        y += 69;
        context.drawTextWithShadow(textRenderer,
                "渲染范围只影响玩家附近的可见部分，不改变球体实际坐标与运算结果。",
                contentX, y, COLOR_MUTED);
        y += 51;
        context.drawTextWithShadow(textRenderer,
                "交集会显示共同区域；并集会显示全部区域；三球后显示模式在前两次只计算不渲染。",
                contentX, y, COLOR_MUTED);
    }
    ...
    private void renderHelpPage(DrawContext context) {
        ...
        y += 18;
        drawHelpLine(context, "250–200、200–150、150–100、100–50、50–25", y, COLOR_TEXT);
        y += 28;
        drawHelpLine(context, "说明", y, COLOR_ORANGE);
        y += 18;
        drawHelpLine(context, "白色半透明实心方块仅为客户端渲染，不会真实放置或破坏方块。", y, COLOR_TEXT);
        y += 15;
        drawHelpLine(context, "设置会保存到 config/skeleton_searcher.json。", y, COLOR_TEXT);
        y += 15;
        drawHelpLine(context, "快捷键可以在“选项 → 控制 → 按键绑定 → Skeleton Searcher”中修改。", y, COLOR_TEXT);
    }
    ...
    private static final class RenderDistanceSlider extends SliderWidget {
        ...
        private static double normalize(int distance) {
            return (Math.clamp(distance, 24, 96) - 24) / 72.0;
        }

        private int distance() {
            return 24 + (int) Math.round(value * 72.0);
        }
